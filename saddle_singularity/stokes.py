"""
Stokes transitions: detect from step function, bisect, forward continuation.

Part A: N(γ̃) on fine grid, detect transitions, bisect to γ̃_c.
Part B: At each γ̃_c find θ_c via set difference; track branches forward.
Part C: |θ_birth - θ_k_nearest| / L(γ̃_c).
"""
...
from qaoa_core import (
    ...
    validate_saddle, all_saddles, dominant_saddle_path, DEFAULT_BETA,
)
...
def find_birth_locations(gt_c, n_before, n_after, eps=0.05*np.pi, beta_=beta):
    ...
    def min_dist_to_set(th, ref_set):
        return min(np.abs(th - s) for s in ref_set) if ref_set else np.inf
...
def track_forward(theta_start, gt_start, gt_end, step, beta_=beta, max_step=0.15):
    ...
    while gt <= gt_end - step * 0.5:
        for _ in range(80):
            ...
            if np.abs(f) < 1e-10 or np.abs(fp) < 1e-14:
                break
            dth = -f / fp
...
def main():
    ...
    # Restrict fine scan to γ̃ up to 2π for testing.
    gts_fine = np.arange(0.15 * np.pi, 2 * np.pi + step_fine/2, step_fine)
    ...
    # Track branches only up to γ̃ = 2π.
    gt_max = 2 * np.pi
    step_fwd = 0.01 * np.pi
    gts_dom, thetas_dom = dominant_saddle_path(gt_max, n_steps=300)
    dom_curve = [(gt, Phi(th, gt, beta).real) for gt, th in zip(gts_dom, thetas_dom)]
    branch_curves = [dom_curve]
    ...
    for gt_c, theta_c, d, L_c in transitions:
        ...
        if path:
            branch_curves.append([(gt, Phi(th, gt, beta).real) for gt, th in path])
    ...
    plt.tight_layout()
    out_path = OUT_DIR / 'stokes_transitions_v3.png'
